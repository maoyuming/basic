...
public class MealElasticsearchUtil {
     ...
     * 默认type
     */
    public final static String ES_TYPE = "meal";
     ...
     * @return
     */
    public BulkResponse batchAddDocument(List<MealInputBean> objList) {
        BulkResponse bulkResponse = null;
        try {
            BulkRequestBuilder requestBuilder = this.getClient().prepareBulk();
            for (MealInputBean obj : objList) {
    ...
    private void initClient() throws IOException {
        try {
            ...
            if (!this.indexExists(ES_INDEX)) {
                ...
                //final XContentBuilder mealMappingBuilder = this.createGeoMappingBuilder();
                //createIndexRequestBuilder.addMapping(ES_TYPE, mealMappingBuilder);
            ...
            if(!this.typeExists(ES_INDEX, ES_TYPE)){
            	...
            	PutMappingResponse response =
            	        client.admin().indices().preparePutMapping(ES_INDEX).setType(ES_TYPE)
            	            .setSource(mealMappingBuilder).execute().actionGet();
            	 if (response.isAcknowledged()) {
            	      logger.info("MealElasticsearchUtil Type and mapping created !{}", ES_TYPE);
        	     } else {
     ...
     * @return
     */
    public SearchHit[] searchMealByMealId(String mealId) {
        SearchHit[] hits = null;
        try {
            SearchRequestBuilder searchBuilder = this.prepareSearch();
            FilterBuilder termFilter = FilterBuilders.termFilter("mealId", mealId);
            ...
            this.logger.error("MealElasticsearchUtil searchMealByMealId error",e);
        }
        return hits;
    }
...
//        hits = new SearchHit[item.getTotalItem()];
    ...
    public List<MealOutputBean> searchMeals(MealQueryBean mealQueryBean) {
        SearchHit[] hits = null;
        List<MealOutputBean> list = new ArrayList<MealOutputBean>();
        try {
            SearchRequestBuilder searchBuilder = this.prepareSearch();
     ...
     * @return
     */
    private List<MealOutputBean> dataTransfer(SearchHit[] hits) {
        List<MealOutputBean> list = new ArrayList<MealOutputBean>();
        for (SearchHit searchHit : hits) {
            Map<String, Object> map = searchHit.getSource();
            MealOutputBean mealOutputBean = dozerMapper.map(map, MealOutputBean.class);
            list.add(mealOutputBean);
        }
        return list;
    }
